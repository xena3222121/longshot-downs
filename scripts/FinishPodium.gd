...
## Flanking cheering figures in the victory beat (AJ: "bikini girls added
## into the victory sequence... the flat stuff") — flat cutout cards, same
## art-fidelity level as everything else in this game (not even the jockeys
## are modeled), generated by generate_podium_girls.gd. Positioned in the
## open flanks either side of the title banner (which spans roughly
## CENTER_X-350 to CENTER_X+350) and well above BASELINE_Y, so they never
## collide with the payoff/outcome panels that build in below a moment
## later. Falls back to nothing if the PNGs are ever missing, same
## optional-asset pattern as every other sourced/generated image this
## project uses.
const PODIUM_GIRL_FILES: Array[String] = ["podium_girl_1.png", "podium_girl_2.png"]
const PODIUM_GIRL_SIZE: Vector2 = Vector2(140.0, 216.0)

func _show_podium_girls() -> void:
	_show_podium_girl(PODIUM_GIRL_FILES[0], CENTER_X - 470.0, -1.0)
	_show_podium_girl(PODIUM_GIRL_FILES[1], CENTER_X + 330.0, 1.0)

func _show_podium_girl(file_name: String, x: float, bob_phase: float) -> void:
	var texture: Texture2D = load("res://assets/textures/" + file_name) if ResourceLoader.exists("res://assets/textures/" + file_name) else null
	if texture == null:
		return
	...
	sprite.custom_minimum_size = PODIUM_GIRL_SIZE
	sprite.size = PODIUM_GIRL_SIZE
	sprite.position = Vector2(x, 70.0)
	sprite.pivot_offset = PODIUM_GIRL_SIZE * 0.5
	sprite.scale = Vector2.ZERO
	sprite.mouse_filter = Control.MOUSE_FILTER_IGNORE
	add_child(sprite)

	var tween: Tween = create_tween()
	tween.tween_property(sprite, "scale", Vector2(1.1, 1.1), 0.4).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	tween.tween_property(sprite, "scale", Vector2(1.0, 1.0), 0.15).set_trans(Tween.TRANS_SINE)

	# A small continuous cheering bob (opposite phase per side so the two
	# don't move in lockstep) — same "loop a couple times, don't leave it
	# wobbling forever next to serious dollar-amount cards" reasoning as the
	# title banner's own wobble tween above.
	var start_y: float = sprite.position.y
	var bob: Tween = create_tween()
	bob.set_loops(3)
	bob.tween_property(sprite, "position:y", start_y - 12.0 * bob_phase, 0.25).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	bob.tween_property(sprite, "position:y", start_y, 0.25).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
